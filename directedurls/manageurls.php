...
$baseurl = new moodle_url('/blocks/directeredurls/manageurls.php');
$editbaseurl = new moodle_url('/blocks/directedurls/editurl.php');
...
$baseurl = new moodle_url('/blocks/directedurls/manageurls.php', $urlparams);
$PAGE->set_url($baseurl);

// Process any actions.
if ($deleteurlid && confirm_sesskey()) {
    $DB->delete_records('block_directedurls', array('id' => $deleteurlid));

}
if ($hideurlid && confirm_sesskey()) {

    if ($hideurl) {
        $DB->update_record('block_directedurls', array('id' => $hideurlid, 'visible' => 0));
    } else {
        $DB->update_record('block_directedurls', array('id' => $hideurlid, 'visible' => 1));
    }
...
if (!empty($moveup) || !empty($movedown)) {
    if (!empty($moveup)) {
        if ($moveurl = $DB->get_record('block_directedurls', array('id' => $moveup))) {
            $swapurl = $DB->get_record('block_directedurls', array('sortorder' => $moveurl->sortorder - 1));
        }
    } else if ($moveurl = $DB->get_record('block_directedurls', array('id' => $movedown))) {
        $swapurl = $DB->get_record('block_directedurls', array('sortorder' => $moveurl->sortorder + 1));
    }

    if ($swapurl and $moveurl) {
        $DB->set_field('block_directedurls', 'sortorder', $swapurl->sortorder, array('id' => $moveurl->id));
        $DB->set_field('block_directedurls', 'sortorder', $moveurl->sortorder, array('id' => $swapurl->id));
    }
...
$links = $DB->get_records_select('block_directedurls', null, null, $DB->sql_order_by_text('sortorder'));

$strmanage = get_string('manageurls', 'block_directedurls');
...
$table = new flexible_table('directedurls-display-links');

$table->define_columns(array('displaytext', 'url',  'actions'));
$table->define_headers(array(get_string('configtableheaderdisplaytext', 'block_directedurls'),
                             get_string('configtableheaderurl', 'block_directedurls'),
...
foreach ($links as $link) {
    ...
    $feedicons .= $OUTPUT->action_icon($deleteurl,
                                $deleteicon,
                                new confirm_action(get_string('deleteurlconfirm', 'block_directedurls')));
...
$url = $CFG->wwwroot . '/blocks/directedurls/editurl.php?' . substr($extraparams, 1);
echo '<div class="actionbuttons">' . $OUTPUT->single_button($url, get_string('addnewurl', 'block_directedurls'), 'get') . '</div>';
